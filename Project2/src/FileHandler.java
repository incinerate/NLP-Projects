...
class FileHandler {
	...
	*/
		...
		while ((cur_line = br.readLine()) != null) {
			if (is_first || cur_line.length() == 0) {
				if (is_first) {
					...
					num_sentences++;
					sent = new Sentence();
					sent.addWord(new Word(cur_line));
	...
	public static void main(String[] args) throws Exception {
		...
		String testFilename = "data/p2/unlabeled_20news.txt";

		FileHandler fh = new FileHandler();

		ArrayList<Sentence> corpus = fh.readTaggedSentences(trainFilename);
		ArrayList<Sentence> tCorpus = fh.readUntaggedSentences(testFilename);
		
		//System.out.println(corpus.get(0).getWordAt(0).getPosTag());
		System.out.println(tCorpus.get(0).toString());
		System.out.println(tCorpus.get(1).toString());
		System.out.println(tCorpus.get(corpus.size() - 1).toString());
	}
}
